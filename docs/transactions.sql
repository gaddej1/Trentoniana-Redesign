...
/*Filter recordings based on number of speakers*/
CREATE VIEW REC_SPEAKER_COUNT AS
SELECT COUNT(Speaker_number), Recording_title
FROM INTERVIEW_WITH
GROUP BY Recording_title;

SELECT Recording_title
FROM REC_SPEAKER_COUNT
WHERE count = 1;

DROP VIEW REC_SPEAKER_COUNT;
